...
public class PageRankSlots {
	...
	public void go(String current_date){
		...
		db.setEdgeQuery("SELECT sender_recipient_dates.sender AS source, sender_recipient_dates.rvalue AS target "
				+ "FROM test.sender_recipient_dates WHERE date like '"+current_date+"'");
...
//		    float y = nodes.getNodeData().y();
		    ...
		    String query = "UPDATE " + db.getDBName() + ".count_slots "
		    		+ "SET pagerank = "+pgrank+" WHERE count_slots.email='" + id+"' and count_slots.date like '"+current_date+"'";
		    try {
