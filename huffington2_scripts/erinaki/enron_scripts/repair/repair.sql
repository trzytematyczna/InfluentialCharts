...
delimiter $$
create function repair_erinaki() returns text
begin
...
where sender = varuser and upper(substr(subject,1,3)) not like 'RE:%';
...
simple_loop: loop
    ...
    and upper(substr(subject,1,3)) = 'RE:' and substr(subject,5,length(subject)) = varsubject
    and date > vardate;
