...
--select count(link_from_content) into outlinks_articles from posts where id = varid and link_from_content like '%%';
--select count(link_from_content) into outlinks_news from posts where id = varid and link_from_content like '%%';
--select count(link_from_content) into outlinks_photo_video from posts where id = varid and link_from_content like '%.jpg%';
...
--select count(distinct author_id) into inlinks_users from comments where id in (select id from comments where post_id= varid);

insert into agarwal_posts_stats values(varauthor, varid, );
